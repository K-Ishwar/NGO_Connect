...
class _CommunityHubScreenState extends State<CommunityHubScreen> {
  ...
  Future<void> _pledge(String surveyId) async {
    ...
    try {
      await FirebaseFirestore.instance.collection('surveys').doc(surveyId).update({
                ...
                children: [
                  ...
                  const Column(
                    ...
                    children: [
                      ...
                      Text('All NGO surveys — Open data sharing',
        ...
        child: StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, snap) {
            ...
            final allSurveys = snap.data!.docs.map((d) {
              return SurveyModel.fromMap(d.data() as Map<String, dynamic>, d.id);
            }).toList();

            // Apply filters
            var filtered = allSurveys.where((s) {
              final matchUrgency = _filterUrgency == 'All' || s.urgency == _filterUrgency;
              final matchProblem = _filterProblem == 'All' || s.problemType == _filterProblem;
              final matchSearch = _searchText.isEmpty ||
                  s.area.toLowerCase().contains(_searchText.toLowerCase()) ||
                  s.description.toLowerCase().contains(_searchText.toLowerCase());
            ...
            filtered.sort((a, b) {
              ...
              final urgCompare = (order[a.urgency] ?? 1).compareTo(order[b.urgency] ?? 1);
              if (urgCompare != 0) return urgCompare;
              return b.date.compareTo(a.date);
            });

            // Aggregate stats
            final totalPeople = allSurveys.fold<int>(0, (sum, s) => sum + s.peopleCount);
            final highCount = allSurveys.where((s) => s.urgency == 'High').length;
            ...
            return Column(
              children: [
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      _statPill('Total Surveys', allSurveys.length.toString(), Icons.assignment, Colors.white),
                      _statDivider(),
                      _statPill('People Affected', _formatNum(totalPeople), Icons.people, Colors.white),
                      _statDivider(),
                      _statPill('High Urgency', highCount.toString(), Icons.warning_amber, Colors.redAccent.shade100),
                      _statDivider(),
                      _statPill('NGOs Active', _countNgos(allSurveys).toString(), Icons.business, Colors.lightBlueAccent.shade100),
                    ],
                  ),
                ),
                  ...
                  child: filtered.isEmpty
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (ctx, i) => _SurveyHubCard(
                            ...
                            onViewAnalytics: () => Navigator.push(context,
                                MaterialPageRoute(builder: (_) => SurveyAnalyticsScreen(survey: filtered[i]))),
                          ),
                        ),
                ),
              ],
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const Text('No surveys match your filters.', style: TextStyle(color: Colors.grey)),
        ],
      ),
    );
  }
  ...
  String _formatNum(int n) => n >= 1000 ? '${(n / 1000).toStringAsFixed(1)}k' : n.toString();

  int _countNgos(List<SurveyModel> surveys) => surveys.map((s) => s.ngoId).toSet().length;
}
...
class _SurveyHubCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final dateStr = '${survey.date.day}/${survey.date.month}/${survey.date.year}';
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        Container(
                          ...
                          child: Row(children: [
                            Icon(Icons.warning_amber, color: urgColor, size: 12),
                            const SizedBox(width: 3),
                            Text(survey.urgency,
                                style: TextStyle(color: urgColor, fontSize: 10, fontWeight: FontWeight.bold)),
                          ]),
                        ),
                      ],
                    ),
                    ...
                    // Area
                    Row(children: [
                      const Icon(Icons.location_on, size: 14, color: Color(0xFF8A2387)),
                      const SizedBox(width: 4),
                      Text(survey.area, style: const TextStyle(color: Colors.deepPurple, fontSize: 13, fontWeight: FontWeight.w600)),
                      const SizedBox(width: 10),
                    ...
                    // Stats row
                    Row(children: [
                      _infoPill(Icons.people, '${survey.peopleCount} affected', Colors.blueAccent),
                      ...
                      FutureBuilder<QuerySnapshot>(
                        ...
                        builder: (ctx, snap) {
                          ...
                          return _infoPill(Icons.assignment_turned_in, '$count responses', Colors.green);
                        },
                      ),
                    ]),
                    const SizedBox(height: 10),
                    ...
                    StreamBuilder<DocumentSnapshot>(
                      stream: FirebaseFirestore.instance
                          .collection('surveys').doc(survey.id).snapshots(),
                      builder: (ctx, surveySnap) {
                        final pledges = surveySnap.hasData && surveySnap.data!.exists
                            ? List<String>.from(
                                (surveySnap.data!.data() as Map)['pledges'] ?? [])
                            : <String>[];
                        final myPledged = pledges.contains(myId);
                        final isMyOwnSurvey = survey.ngoId == myId;
                        final ngoName = survey.ngoName.isNotEmpty ? survey.ngoName : 'NGO';
                              ...
                              children: [
                                ...
                                Row(children: [
                                  Container(
                                    ...
                                    child: Row(children: [
                                      const Icon(Icons.business, size: 11, color: Colors.deepPurple),
                                      const SizedBox(width: 4),
                                      Text(ngoName,
                                    ...
                                    child: GestureDetector(
                                      ...
                                      child: Container(
                                        ...
                                        child: const Row(
                                          mainAxisAlignment: MainAxisAlignment.center,
                                          children: [
                                            Icon(Icons.bar_chart, color: Color(0xFF8A2387), size: 16),
                                            SizedBox(width: 6),
                                            Text('View Data', style: TextStyle(color: Color(0xFF8A2387), fontWeight: FontWeight.bold, fontSize: 12)),
                                          ],
                                        ),
                                      ),
                                    ),
                                      ...
                                      child: GestureDetector(
                                        ...
                                        child: Container(
                                          ...
                                          decoration: BoxDecoration(
                                            ...
                                            color: myPledged ? Colors.green.shade50 : null,
                                            borderRadius: BorderRadius.circular(10),
                                            border: myPledged
                                          ...
                                          child: Row(
                                            mainAxisAlignment: MainAxisAlignment.center,
                                            children: [
                                              Icon(myPledged ? Icons.check_circle : Icons.handshake,
                                                  color: myPledged ? Colors.green : Colors.white, size: 16),
                                              const SizedBox(width: 6),
                                              Text(myPledged ? 'Pledged ✓' : 'Pledge Support',
                                                  style: TextStyle(
                                                    color: myPledged ? Colors.green : Colors.white,
                                      ...
                                      child: Container(
                                        ...
                                        child: const Row(
                                          ...
                                          children: [
                                            ...
                                            SizedBox(width: 6),
                                            Text('Your Survey', style: TextStyle(color: Colors.blue, fontWeight: FontWeight.bold, fontSize: 12)),
                                          ],
                                        ),
                                      ),
                                    ),
